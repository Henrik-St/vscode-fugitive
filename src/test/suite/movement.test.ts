...
export async function close(): Promise<void> {
    ...
    assert(
        vscode.window.activeTextEditor?.document.uri.scheme !== 'fugitive',
        "Fugitive document is still open"
    );
}
export async function goTop(): Promise<void> {
    ...
    assert.strictEqual(
        vscode.window.activeTextEditor?.selection.active.line, 
        0, 
        "Cursor does not got to the top"
    );
}
export async function goDown(): Promise<void> {
    ...
    assert.strictEqual(
        vscode.window.activeTextEditor?.selection.active.line, 
        6, 
        "Cursor does not go down one line"
    );
}
export async function goUp(): Promise<void> {
    ...
    assert.strictEqual(
        vscode.window.activeTextEditor?.selection.active.line, 
        4, 
        "Cursor does not go up one line"
    );
}
